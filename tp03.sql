...
SELECT *
FROM ARTICLE
WHERE ID_FOU IN (SELECT ID FROM FOURNISSEUR WHERE NOM = 'Française d''Imports' OR NOM = 'Dubois et Fils'); -- OR = WHERE ID_FOU IN(1,3);

SELECT *
FROM ARTICLE
WHERE ID_FOU NOT IN (SELECT ID FROM FOURNISSEUR WHERE NOM IN ('Française d''Imports', 'Dubois et Fils'));-- OR = WHERE ID_FOU NOT IN(1,3);
